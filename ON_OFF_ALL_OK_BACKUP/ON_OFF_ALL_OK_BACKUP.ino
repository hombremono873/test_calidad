...
const int maquinas[] = {13, 12, 14, 27, 26, 25, 33, 32};   //Array con el numero del pin al cual se conectan las cargas a contolar (GPIO)
const int numMaquinas = sizeof(maquinas) / sizeof(maquinas[0]);  //Calculo del número de cargas

WiFiServer server(80);
// Nombre  o tag asinado a cada carga 
...
//Se configuran los pins o patillas del ESP32 como salidas, se programa velocidad de programacion 
void setup() {
  Serial.begin(115200);
  delay(10);
...
void procesarPeticion(const String& peticion) {
  ...
  for (int i = 0; i < numMaquinas; i++) {
    ...
    // Lógica adicional para el Horno FC01  //El horno se hace  ON si hay prendida una de las bombas de agua y el ventilador de refrigeración
